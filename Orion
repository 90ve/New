...
function OrionLib:MakeWindow(WindowConfig)
	...
	
	{
		...
		BackgroundColor = BrickColor.new(40, 0, 0),
	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddToggle(ToggleConfig)
				...
				local ToggleBox = SetChildren(SetProps(MakeElement("RoundFrame", ToggleConfig.Color, 0, 12), {
...
function ElementFunction:AddToggleSlider(Config)
	Config = Config or {}
	Config.Name = Config.Name or "Toggle Slider"
	...
	Config.Color = Config.Color or Color3.fromRGB(9, 149, 98)
	...
	--================ MAIN FRAME =================
	local MainFrame = AddThemeObject(SetChildren(SetProps(
	...
	--================ TOGGLE BOX =================
	local ToggleBox = SetChildren(SetProps(
		MakeElement("RoundFrame", Config.Color, 0, 12),
		{
			Size = UDim2.new(0, 24, 0, 24),
	...
	local SliderDrag = SetChildren(SetProps(
		MakeElement("RoundFrame", Config.Color, 0, 5),
		{
	...
	-- Slider stops BEFORE toggle (important fix)
	local SliderBar = SetChildren(SetProps(
		MakeElement("RoundFrame", Config.Color, 0, 5),
		{
			Size = UDim2.new(1, -54, 0, 26), -- leaves space for toggle
	...
	function Control:SetEnabled(v)
		...
		TweenService:Create(ToggleBox, TweenInfo.new(0.25), {
			BackgroundColor3 = v and Config.Color or OrionLib.Themes.Default.Divider
		}):Play()

		TweenService:Create(ToggleBox.Stroke, TweenInfo.new(0.25), {
			Color = v and Config.Color or OrionLib.Themes.Default.Stroke
		}):Play()

		ToggleBox.Ico.ImageTransparency = v and 0 or 1
		SliderBar.BackgroundTransparency = v and 0.9 or 0.97

		Config.Callback(Control.Enabled, Control.Value)
	end

	function Control:SetValue(v)
		Control.Value = math.clamp(
			Round(v, Config.Increment),
			Config.Min,
			Config.Max
		)
		...
		TweenService:Create(SliderDrag, TweenInfo.new(.15), {
			Size = UDim2.fromScale(
				(Control.Value - Config.Min) / (Config.Max - Config.Min),
				1
			)
		}):Play()

		SliderBar.Value.Text = tostring(Control.Value) .. " " .. Config.ValueName
		SliderDrag.Value.Text = SliderBar.Value.Text
	...
	-- Slider drag ONLY controls value
	...
	AddConnection(UserInputService.InputChanged, function(Input)
		if Dragging and Control.Enabled then
			...
			Control:SetValue(
				Config.Min + (Config.Max - Config.Min) * scale
			)
	...
	-- Toggle ONLY toggles when clicking toggle button
	AddConnection(ToggleBox.InputBegan, function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1
		or Input.UserInputType == Enum.UserInputType.Touch then
			Control:SetEnabled(not Control.Enabled)
			SaveCfg(game.GameId)
		end
	end)
	...
	if Config.Flag then
		OrionLib.Flags[Config.Flag] = Control
	end
